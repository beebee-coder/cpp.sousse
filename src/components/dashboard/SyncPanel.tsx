...
import { RefreshCw, Cloud, CloudOff, Wifi, WifiOff, CheckCircle2, AlertCircle, UploadCloud, DownloadCloud, ArrowDownUp, Laptop, Database, Loader2 } from 'lucide-react';
...
export function SyncPanel() {
  ...
  const showWebToHybride = mode === 'hybride';
  const showHybrideToLocale = mode === 'hybride';
  const syncBlocked = cloudDisabled || isSyncing;

  const showVectorize = mode === 'hybride' && !!selection.relPath;
      ...
      {showWebToHybride && (
        ...
        >
          ...
          <span className="text-[10px] font-bold uppercase font-headline tracking-widest">
            {isSyncing ? "Sync en cours..." : cloudDisabled ? "Sync suspendu" : "Sync Web → Hybride"}
          </span>
        </button>
      )}
        ...
        <button
          ...
          className={cn(
            ...
            isSyncing
              ...
              : cloudDisabled
                ? "border-muted/20 text-muted-foreground/40 cursor-not-allowed"
                : "border-secondary/40 text-secondary hover:bg-secondary/5 hover:border-secondary"
          )}
        >
          <Laptop className={cn("w-3 h-3", isSyncing && "animate-spin")} />
          <span className="text-[10px] font-bold uppercase font-headline tracking-widest">
            {isSyncing ? "Sync en cours..." : cloudDisabled ? "Sync suspendu" : "Sync Hybride → Locale"}
          </span>
        </button>
      )}
