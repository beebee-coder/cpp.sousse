...
export function ProcedureExecutor({ procedure }: ProcedureExecutorProps) {
  ...
  // Mise à jour de l'horloge interne pour les minuteurs
  useEffect(() => {
  ...
  const handleNext = () => setState(engine.nextStep());
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 h-full">
      ...
      <div className="lg:col-span-3 space-y-6">
        {/* Status Header */}
        <Card className="p-4 border-primary/20 bg-black/40 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <div className="text-right">
              <p className="text-[10px] font-bold text-muted-foreground uppercase">Temps Total</p>
            ...
            <div className="text-right">
              <p className="text-[10px] font-bold text-muted-foreground uppercase">Progression</p>
              <p className="text-sm font-code font-bold text-secondary">{Math.round(progress)}%</p>
            </div>
          </div>
        </Card>
        ...
        <div className="min-h-[500px]">
          ...
          {state.status === 'PREREQUISITES_CHECK' && (
            <Card className="p-8 border-primary/20 bg-card/40 space-y-6">
          ...
          {state.status === 'COMPLETED' && (
            <Card className="h-full flex flex-col items-center justify-center p-12 text-center bg-secondary/5 border-secondary/20">
              ...
              <p className="text-muted-foreground font-code text-sm max-w-md mb-8 uppercase">
                Toutes les étapes ont été validées. Le rapport d'exécution est en cours de génération.
              </p>
              <div className="flex gap-4">
                <Button variant="outline" onClick={() => window.location.reload()} className="h-10 text-[10px] uppercase font-bold">
                  <RotateCcw className="w-4 h-4 mr-2" /> Redémarrer
                </Button>
                <Button className="h-10 text-[10px] uppercase font-bold bg-primary text-primary-foreground">
      ...
      {/* Colonne Latérale : Tracker & Infos */}
      <aside className="space-y-6">
        ...
        <Card className="p-4 border-border bg-black/20 space-y-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h4 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Paramètres Système</h4>
          </div>
          <div className="space-y-2">
            <div className="flex justify-between text-[10px] font-code">
              <span className="text-muted-foreground">ID_PROC</span>
              <span className="text-white">{procedure.id}</span>
            </div>
            <div className="flex justify-between text-[10px] font-code">
              <span className="text-muted-foreground">VERSION</span>
              <span className="text-white">{procedure.metadata.version}</span>
            </div>
            <div className="flex justify-between text-[10px] font-code">
              <span className="text-muted-foreground">OPÉRATEUR</span>
              <span className="text-secondary font-bold">ADMIN_STATION</span>
            </div>
          </div>
        </Card>

        {state.status === 'ALARM' && (
          <Card className="p-4 border-destructive/50 bg-destructive/10 animate-pulse">
            <div className="flex items-center gap-2 mb-2">
              <AlertTriangle className="w-4 h-4 text-destructive" />
              <h4 className="text-[10px] font-bold uppercase tracking-widest text-destructive">Alerte Critique</h4>
            </div>
            <p className="text-[11px] font-code leading-tight text-white uppercase italic">
              Conditions de sécurité non respectées. Vérifiez les paramètres techniques.
            </p>
          </Card>
        )}
      </aside>
