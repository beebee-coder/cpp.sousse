...
export default function DatasetPage() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onerror = () => {
      ...
      toast({ title: "Erreur", description: "Échec de lecture du fichier", variant: "destructive" });
    };
    ...
    reader.onloadend = async () => {
      try {
        ...
        console.error("Échec traitement média:", err);
        toast({ title: "Erreur", description: "Impossible de traiter ce média. Fichier trop lourd ?", variant: "destructive" });
      } finally {
        setIsProcessingImage(false);
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full overflow-y-auto terminal-scroll">
        <header className="h-16 border-b border-border bg-card/30 flex items-center justify-between px-6 shrink-0">
          ...
          <div className="flex bg-muted/30 p-1 rounded-sm border border-border">
            <button onClick={() => setMode('qa')} className={cn("flex items-center gap-2 px-3 py-1.5 text-[9px] font-code uppercase rounded-sm transition-all", mode === 'qa' ? "bg-primary text-primary-foreground" : "text-muted-foreground")}>
              <MessageSquare className="w-3 h-3" /> Q / R
            </button>
            <button onClick={() => setMode('procedure')} className={cn("flex items-center gap-2 px-3 py-1.5 text-[9px] font-code uppercase rounded-sm transition-all", mode === 'procedure' ? "bg-secondary text-secondary-foreground" : "text-muted-foreground")}>
              <ListOrdered className="w-3 h-3" /> Procédure
            </button>
          </div>
        </header>
        ...
        <div className="p-4 lg:p-8 max-w-5xl mx-auto w-full space-y-6">
          {isProcessingImage && (
            <Card className="p-3 border-primary/20 bg-primary/5 flex items-center gap-3 rounded-sm animate-pulse">
              ...
              <p className="text-[9px] font-code uppercase text-primary leading-tight">Optimisation du flux visuel en cours...</p>
            </Card>
          )}
          ...
          <Card className="p-4 lg:p-6 border-border bg-card/50 shadow-xl space-y-6 rounded-sm">
            ...
            <form onSubmit={handleAddItem} className="space-y-6">
              {mode === 'qa' ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Textarea value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="Ex: Vibration anormale moteur P-02" className="w-full h-32 bg-background/50 border-border rounded-sm p-3 font-code text-xs uppercase" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[9px] font-bold uppercase text-muted-foreground ml-1">Réponse / Action</label>
                    <Textarea value={answer} onChange={(e) => setAnswer(e.target.value)} placeholder="Ex: Vérifier l'alignement de l'arbre..." className="w-full h-32 bg-background/50 border-border rounded-sm p-3 font-code text-xs uppercase" />
                  </div>
                </div>
              ) : (
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <Input value={procTitle} onChange={(e) => setProcTitle(e.target.value)} placeholder="MAINTENANCE_PRÉVENTIVE" className="bg-background/50 border-border font-headline uppercase text-sm h-11" />
                  </div>
                  ...
                  <div className="space-y-4">
                    {procSteps.map((step, index) => (
                      <Card key={index} className="p-4 border-border bg-black/30 space-y-4 relative group">
                        <div className="flex items-center justify-between border-b border-border/50 pb-2">
                          ...
                          <Button type="button" variant="ghost" size="icon" onClick={() => removeStep(index)} className="h-6 w-6 text-destructive opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                          ...
                          <div className="space-y-3 lg:col-span-1 bg-black/20 p-3 rounded-sm border border-border/30">
                            ...
                            <div className="flex gap-2 mb-3">
                              <Button type="button" variant="outline" size="sm" className="flex-1 h-8 text-[9px] font-code uppercase border-primary/40 text-primary" onClick={(e) => handleTriggerCapture(e, index, 'image')} disabled={isProcessingImage}>
                                {isProcessingImage ? <Loader2 className="w-3 h-3 animate-spin" /> : <Camera className="w-3 h-3 mr-2" />} {step.imageUrl ? "Changer" : "Photo"}
                              </Button>
                              <Button type="button" variant="outline" size="sm" className="flex-1 h-8 text-[9px] font-code uppercase border-secondary/40 text-secondary" onClick={(e) => handleTriggerCapture(e, index, 'video')}>
                                <Video className="w-3 h-3 mr-2" /> {step.videoUrl ? "Changer" : "Vidéo"}
                              </Button>
                            </div>
                            <div className="space-y-1">
                              {step.imageUrl && (
                                <div className="flex items-center justify-between p-1.5 bg-background/40 rounded-sm">
                                  <button type="button" onClick={() => setPreviewMedia({ url: step.imageUrl!, type: 'image' })} className="text-[8px] font-code text-primary uppercase truncate flex items-center gap-1 hover:underline group">
                                    <ImageIcon className="w-2.5 h-2.5 shrink-0" /> Visualiser_Asset <Eye className="w-2.5 h-2.5 ml-1 opacity-50 group-hover:opacity-100" />
                                  </button>
                                  <button type="button" onClick={() => deleteMedia(index, 'image')} className="text-destructive p-1 hover:bg-destructive/10 rounded"><X className="w-2.5 h-2.5" /></button>
                                </div>
                              )}
                              {step.videoUrl && (
                                <div className="flex items-center justify-between p-1.5 bg-background/40 rounded-sm">
                                  <button type="button" onClick={() => setPreviewMedia({ url: step.videoUrl!, type: 'video' })} className="text-[8px] font-code text-secondary uppercase truncate flex items-center gap-1 hover:underline group">
                                    <Video className="w-2.5 h-2.5 shrink-0" /> Visualiser_Séquence <Eye className="w-2.5 h-2.5 ml-1 opacity-50 group-hover:opacity-100" />
                                  </button>
                                  <button type="button" onClick={() => deleteMedia(index, 'video')} className="text-destructive p-1 hover:bg-destructive/10 rounded"><X className="w-2.5 h-2.5" /></button>
                                </div>
                              )}
                            </div>
                          </div>
                    ...
                    <Button type="button" variant="outline" onClick={addStep} className="w-full border-dashed border-muted-foreground/30 text-muted-foreground hover:text-primary h-9 text-[10px] uppercase font-code">
              ...
              <div className="flex justify-end pt-4">
                <Button type="submit" size="sm" className={cn("font-headline font-bold uppercase text-[10px] h-9 px-8 shadow-lg", mode === 'qa' ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground")}>
          ...
          <div className="space-y-4 pb-12">
            <div className="flex justify-between items-center px-2">
              <h3 className="text-[10px] lg:text-xs font-bold uppercase tracking-widest font-headline text-muted-foreground flex items-center gap-2">
                <Layers className="w-4 h-4 text-primary" /> Items en attente ({qaItems.length})
              </h3>
              {qaItems.length > 0 && (
              ...
              {qaItems.map((item) => (
                <Card key={item.id} className="p-4 border-border bg-black/20 font-code text-[10px] flex justify-between items-center group rounded-sm hover:border-primary/30 transition-colors">
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    <div className={cn("h-8 w-8 rounded-sm flex items-center justify-center shrink-0 border", item.type === 'procedure' ? "bg-secondary/10 border-secondary/20" : "bg-primary/10 border-primary/20")}>
                    ...
                    <div className="truncate">
                      ...
                      {item.mediaAssets && item.mediaAssets.length > 0 && <Badge variant="outline" className="ml-3 text-[7px] border-yellow-500/50 text-yellow-500 bg-yellow-500/5">{item.mediaAssets.length} MÉDIAS (WEB_BUFFER)</Badge>}
                    </div>
                  </div>
        ...
        <DialogContent className="max-w-3xl bg-card border-primary/30 text-foreground sm:rounded-sm">
          <DialogHeader className="border-b border-border pb-2">
            <DialogTitle className="text-xs uppercase font-headline tracking-widest flex items-center gap-2">
              <Eye className="w-4 h-4 text-primary" /> Vérification Opérateur
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="mt-4 flex justify-end gap-2">
            <Button size="sm" variant="outline" onClick={() => setPreviewMedia(null)} className="h-8 text-[10px] font-code uppercase border-border">Fermer</Button>
            <Button size="sm" onClick={() => setPreviewMedia(null)} className="h-8 text-[10px] font-code uppercase bg-primary text-primary-foreground"><Check className="w-3 h-3 mr-2" /> Confirmer</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
