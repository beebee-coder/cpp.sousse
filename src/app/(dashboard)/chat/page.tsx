...
export default function ChatPage() {
  ...
  const handleManualSubmit = (e?: React.FormEvent) => {
    ...
    if (input.trim() && !isLoading && !voiceRef.current?.isSpeaking) {
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full">
        <header className="h-16 border-b border-border bg-card/30 flex items-center justify-between px-6 shrink-0">
          ...
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" onClick={toggleAudio} className={cn("h-8 text-[9px] uppercase font-code", autoSpeak ? "text-secondary" : "text-muted-foreground")}>
          ...
          <div className="flex-1 flex flex-col gap-4 h-full">
            ...
            <Card className="p-1.5 sm:p-2 border-primary/30 bg-black/60 shadow-2xl shrink-0">
              <form onSubmit={handleManualSubmit} className="flex gap-2">
                ...
                />
                <Button type="submit" size="icon" disabled={isLoading || !input.trim() || voice.isSpeaking} className="bg-primary text-primary-foreground hover:bg-primary/90 h-9 w-9 sm:h-10 sm:w-10">
